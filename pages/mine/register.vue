<template>
	<view class="register">
		...
		<form class="form" @submit="formSubmit">
			<input type="text" name="userAccount" placeholder="请输入帐号" />
			<input password name="password" type="text" placeholder="请输入密码" />
			<button class="submit-btn" form-type="submit">立即注册</button>
		</form>
		<button class="back-btn" @click="toBack()">
			<text class="icon-back iconfont"></text>
		</button>
	</view>
</template>
...
<script>
	export default {
		data(){
			return{
				regRules: [
					{name: 'userAccount',type: 'required',errmsg: '请输入帐号'},
			...
			formSubmit(e) {
				...
				this.$Http.userRegister(e.detail.value).then(res => {
					...
					if(res.code === 1) {
						...
						// uni.showModel({title:'提示',content:})
					}
